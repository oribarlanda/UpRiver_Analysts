...
הפרויקט מאומת באמצעות `npm test`, `npm run lint`, בדיקת TypeScript ו-
`npm run build`. בדיקות פידי היומן הן בדיקות מקומיות ואינן דורשות חיבור
ל-Google או פנייה לרשת.

## 14. חיבור Google Calendar באמצעות subscription

אין צורך ב-Google Cloud, ב-OAuth, ב-Calendar API, ב-refresh tokens, ב-secrets
חדשים או במיגרציה ל-Supabase. האפליקציה מפרסמת פיד ICS קבוע לקריאה בלבד
לכל עובדת ופיד נוסף למנהל. הכתובות כוללות חתימה לא-ניחושת שנגזרת מ-
`SESSION_SECRET` הקיים; לכן חשוב לא לשנות אותו לאחר חיבור היומנים.

כל עובדת מעתיקה את הקישור במסך של שבוע שפורסם. המנהל מוצא את הקישור שלו
ב-`הגדרות` תחת `יומן מנהל`. החיבור מתבצע פעם אחת בדפדפן במחשב:

1. פותחים Google Calendar.
2. ליד `Other calendars / יומנים אחרים` לוחצים `+`.
3. בוחרים `From URL / מכתובת URL`.
4. מדביקים את הקישור שהועתק מהאתר ולוחצים `Add calendar`.

לאחר מכן היומן מופיע גם באפליקציה בטלפון ומתעדכן לפי קצב הרענון של Google.
